...
export default class AuxiliaryLine {
    ...
    private map: L.Map;
    private layer: L.Polyline | null = null;
    // 图层初始化时
    ...
    private initLayers() {
        ...
        this.layer = L.polyline([[181, 181], [182, 182]], polylineOptions);
    ...
    private renderLayer(coords: number[][], valid: boolean = true) {
        if (this.layer) {
            this.layer.setStyle(valid ? this.drawLayerStyle : this.errorDrawLayerStyle);
    ...
    public getGeoJSON(precision?: number | false) {
        if (this.layer) {
            return this.layer.toGeoJSON(precision);
        } else {

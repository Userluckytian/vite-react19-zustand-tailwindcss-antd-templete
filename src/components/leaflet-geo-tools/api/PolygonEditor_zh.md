# PolygonEditor - LeafLetGeoTools 多边形编辑器

## 概述

`PolygonEditor` 是 LeafLetGeoTools 库中用于绘制和编辑多边形的综合组件。它提供高级的多边形创建和操作功能，包括顶点拖动、中点插入、边拖动、顶点删除、多边形拖动、吸附、撤销/重做、验证和多多边形支持。
...
- **自定义样式**：为不同状态配置可自定义的多边形样式
- **可见性控制**：显示/隐藏多边形图层
- **GeoJSON 导出**：将多边形导出为 GeoJSON 格式
- **事件回调**：丰富的状态变更事件系统
- **孔洞支持**：创建和编辑多边形孔洞
...
polygonEditor.onStateChange((state) => {
    ...
    if (state === EditorState.Idle) {
        ...
        console.log('多边形 GeoJSON:', geoJson);
    }
});
```
...
interface LeafletEditorOptions {
    ...
    defaultStyle?: LeafletPolygonOptions; // 默认多边形样式
...
const validationOptions: ValidationOptions = {
    ...
    validErrorPolygonStyle: {          // 无效多边形的错误样式
        color: '#ff0000',
        weight: 3,
        dashArray: '5,5',
        fillOpacity: 0.2
...
polygonEditor.onStateChange((state) => {
    switch (state) {
        case EditorState.Drawing:
            console.log('用户正在绘制多边形');
            break;
        case EditorState.Editing:
            console.log('用户正在编辑多边形');
            break;
        case EditorState.Idle:
            console.log('多边形绘制/编辑完成');
            // 获取结果
            const layer = polygonEditor.getLayer();
            const geoJson = polygonEditor.getGeoJSON();
            console.log('多边形图层:', layer);
            console.log('多边形 GeoJSON:', geoJson);
            break;
    }
});
...
3. **拖动中点**：插入新顶点
4. **拖动边**：移动整个边段
5. **拖动多边形**：移动整个多边形（点击并拖动内部）
6. **右键单击顶点**：删除顶点（最少需要 3 个顶点）
7. **双击**：提交更改并退出编辑模式

...
// 地图类型定义
type MapType = "normal" | "earth" | "satellite" | "traffic";
// 绘制类型定义
type DrawingType =
  | "marker"
  | "polyline"
  ...
  const switchMapType = (mapType: MapType) => {
    ...
    try {
      switch (mapType) {
        ...
        case "traffic":
          // 全景地图 - 使用数字常量
          map.setMapType((window as any).BMAP_PERSPECTIVE_MAP || 3);
          map.setTilt(45); // 全景地图通常需要设置俯仰角来显示3D效果
      }
    ...
    <div className="map-container">
      ...
      {/* <div className="map-type-control">
        <div className="control-header">
          <span className="title">地图类型</span>
          <span className="current-type">
            {mapTypes.find((m) => m.key === currentMapType)?.name}
          </span>
        </div>
          ...
          {mapTypes.map((mapType) => (
            <button
              ...
              className={`type-button ${
                currentMapType === mapType.key ? "active" : ""
              }`}
              onClick={() => switchMapType(mapType.key)}
              title={mapType.description}
            >
              <span className="icon">{mapType.icon}</span>
              <span className="name">{mapType.name}</span>
            </button>
          ))}
        </div>
      </div> */}
      ...
      {/* <div className="lnglat">
        ...
        <span className="coordinate">
          {(lnglat && formatNumber(lnglat.lng, 3)) || 116.403}
        </span>
        <span>纬度：</span>
        <span className="coordinate">
          {(lnglat && formatNumber(lnglat.lat, 3)) || 39.928}
        </span>
        <span className="map-type-info">
          模式: {mapTypes.find((m) => m.key === currentMapType)?.name}
        </span>
      </div> */}
    </div>
  );
